...
namespace LameScooter {
    ...
    public class OfflineLameScooterRental : ILameScooterRental {
        private List<LameScooterStation> ReadOffineFile() {
            var fileName = "scooters.json";
            var path = Path.Combine(Environment.CurrentDirectory, @"OffineData", fileName);
            try {
                using var sr = new StreamReader(path);
                var data = sr.ReadToEnd();
                var result = JsonSerializer.Deserialize<List<LameScooterStation>>(data);
                return result;
            }
            catch(IOException e) {
        ...
        public Task<int> GetScooterCountInStation(string stationName) {
            try {
                ...
                var station = data as LameScooterStation[] ?? data.ToArray();
